...
def load_count_data() -> pd.DataFrame:
    ...
    df: pd.DataFrame = (
        df.groupby(["Station_id", "Date", "Lane_Name"])
        .aggregate({"Station_Name": "first", "Count": "sum", "Average_Speed": "mean", "Average_Temperature": "mean"})
        .reset_index()
    )  # type: ignore
    # We also want to parse the date as date rather than string, for filtering purposes
    df["Date"] = df["Date"].map(lambda x: dt.datetime.strptime(x, "%Y-%m-%d").date())

    return df


def date_filter(
    locations: pd.DataFrame, data: pd.DataFrame
) -> tuple[tuple[dt.date, dt.date], pd.DataFrame, pd.DataFrame]:
    min: date = data["Date"].min()  # type: ignore
    max: date = data["Date"].max()  # type: ignore
    date_slider: tuple[dt.date, dt.date] = st.slider("Dato/tid", min, max, (min, max))
    st.info(
        "Valgbar tidsramme bestemmes av underliggende datasett. Se mer info under [referansedata](/#referansedata).",
        icon="ℹ️",
    )
...
def render_overview_map(locations: pd.DataFrame):
    ...
    map_fig = px.scatter_mapbox(
        ...
        size="Count",
        color="Count",
        labels={"Count": "Antall passeringer"},
    )
...
def render_specific_location(locations: pd.DataFrame, data: pd.DataFrame):
    ...
    sel_id = st.selectbox(
        ...
        options,
        index=sel_ix,
        format_func=lambda id: f'{loc_dict[id]["Navn"].item()} ({int(loc_dict[id]["Count"].item())} passeringer)',
        key="sel_id",
    )
    if sel_id == None:
        return
...
def render_data_tables(locations: pd.DataFrame, data: pd.DataFrame):
    ...
    with st.expander("Referansedata"):
        ...
        st.info(
            "Datasettet er transformert noe i forhold til opprinnelig csv for å tilpasses denne nettsidens behov",
            icon="ℹ️",
        )
        st.write(data)
...
def app():
    ...
    render_overview_map(locations_with_counts)
